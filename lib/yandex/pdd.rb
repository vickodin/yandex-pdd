module Yandex
  class Pdd
    ...
    # user_oauth_token
    # https://pddimp.yandex.ru/api/user_oauth_token.xml ? token=<токен> & domain=<имя домена> & login=<имя пользователя>
    
    def user_oauth_token(login, domain)
    ...
    # http://passport.yandex.ru/passport ? mode=oauth & type=trusted-pdd-partner & error_retpath=<URL для редиректа> & access_token=<токен>
